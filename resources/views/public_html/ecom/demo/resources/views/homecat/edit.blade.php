                                    ...
                                    <div class="d-flex align-items-baseline flex-wrap mr-5">
                                        ...
                                        <ul class="breadcrumb breadcrumb-transparent breadcrumb-dot font-weight-bold p-0 my-2 font-size-sm">
                                            <li class="breadcrumb-item text-muted">
                                                <a href="{{ route('admin-homecat') }}" class="text-muted">List of Homepage Category</a>
                                            </li>
                                        </ul>
                                    </div>
                                    <!--end::Page Heading-->
                                ...
                                <div class="row">
                                    <div class="col-md-12">
                                        ...
                                        <div class="card card-custom gutter-b example example-compact">
                                            ...
                                            <form method="POST" action="{{route('admin-homecat-update',$data->id)}}" onsubmit="return validation();" id="formEdit">
                                                {{ csrf_field() }}
                                                <div class="card-body">
                                                    ...
                                                    <div class="form-group row">
														...
														<div class="col-10 col-form-label">
															<div class="radio-inline">
																...
																<input type="radio" name="radios5" id="style_1" value="1" {{ ($data->styles=="1")? "checked" : "" }} required>
																<span></span>Style 1</label>
																<label class="radio radio-success">
																<input type="radio" name="radios5" id="style_2" value="2"  {{ ($data->styles=="2")? "checked" : "" }}>
																<span></span>Style 2</label>
																<label class="radio radio-success">
																<input type="radio" name="radios5" id="style_3" value="3"  {{ ($data->styles=="3")? "checked" : "" }}>
                                                         ...
                                                         <div class="col-10">
                                                            <select id="choices-multiple-remove-button" name="category[]" multiple required>
                                                                @foreach ($data1 as $item)
                                                                    <option value="{{$item->id}}" {{ (array_key_exists($item->id,$product2))?'selected':'' }}>{{ $item->category_name }}</option>
                                                                @endforeach
                                                            </select> 
                                                        </div>
                                                    </div>
...
$("select").on('click', 'option', function() {
    ...
    if ($("select option:selected").length > 5) {
     ...
     var array2 = [];
     var array  =  {!! json_encode($product2) !!};
    ...
    $.post('{{route('admin-homecat-load')}}',{"_token": "{{ csrf_token() }}",data:$("#choices-multiple-remove-button").val()},function(data,status){
            $('#table').html(data);
